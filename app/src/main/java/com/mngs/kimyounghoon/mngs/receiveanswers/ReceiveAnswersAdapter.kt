...
import com.mngs.kimyounghoon.mngs.databinding.ItemEmptyReceiveAnswerBinding
...
class ReceiveAnswersAdapter(private val userActionListener: LocateListener, var answers: List<Answer> = ArrayList(), var isAllLoaded: Boolean = false) : BaseRecyclerAdapter() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            Constants.EMPTY_ITEM -> {
                val binding = ItemEmptyReceiveAnswerBinding.inflate(LayoutInflater.from(parent.context), parent, false)
                ReceiveAnswersEmptyViewHolder(binding)
            }
            Constants.LOAD_MORE
            -> {
                ...
                val binding = ItemReceiveAnswerBinding.inflate(LayoutInflater.from(parent.context), parent, false)
                ReceiveAnswersViewHolder(binding)
            }
        }
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ReceiveAnswersViewHolder) {
